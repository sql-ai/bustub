 ...
 * or will come from the child of the InsertPlanNode. 
 * To simplify the assignment, InsertPlanNode has at most one child.
 */
class InsertPlanNode : public AbstractPlanNode 
{
 public:
  ...
  table_oid_t TableOid() const 
  { 
    return table_oid_;
  }

  /** @return true if we embed insert values directly into the plan, 
   * false if we have a child plan providing tuples 
   **/
  bool IsRawInsert() const 
  { 
