...
class HashJoinPlanNode : public AbstractPlanNode 
{
 public:
  ...
  const AbstractExpression *Predicate() const 
  { 
    return predicate_; 
  }
  ...
  const AbstractExpression *GetLeftKeyAt(uint32_t idx) const 
  { 
    return left_hash_keys_[idx]; 
  }

  /** @return the left keys */
  const std::vector<const AbstractExpression *> &GetLeftKeys() const 
  { 
    return left_hash_keys_; 
  }

  /** @return the right key at the given index */
  const AbstractExpression *GetRightKeyAt(uint32_t idx) const 
  { 
    return right_hash_keys_[idx]; 
  }

  /** @return the right keys */
  const std::vector<const AbstractExpression *> &GetRightKeys() const 
  { 
