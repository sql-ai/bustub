    ...
    : buffer_pool_manager_(buffer_pool_manager), comparator_(comparator), hash_fn_(std::move(hash_fn)), num_buckets_(num_buckets), size_(0)
{
  ...
  for (size_t i = 0; i < num_blocks_; i++) {
    ...
    LOG_DEBUG("Initial block ids: %d", int(block_page_id));
...
bool HASH_TABLE_TYPE::Insert(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  for(size_t i = 0; i < num_buckets_; i++)
  {
    ...
    if(block_page->IsReadable(bucket_idx) && 
        comparator_(key, block_page->KeyAt(bucket_idx)) == 0 && value == block_page->ValueAt(bucket_idx))
    {
...
void HASH_TABLE_TYPE::Resize(size_t initial_size) 
{
  ...
  HashTableHeaderPage* old_header = reinterpret_cast<HashTableHeaderPage *>(buffer_pool_manager_->FetchPage(old_page_id));
  
  num_buckets_ = initial_size * 2;
  num_blocks_ = (num_buckets_-1) / BLOCK_ARRAY_SIZE + 1;

  HashTableHeaderPage* header_page = reinterpret_cast<HashTableHeaderPage *>(buffer_pool_manager_->NewPage(&header_page_id_));
  ...
  for(size_t block_idx = 0; block_idx < old_header->NumBlocks(); block_idx++) {
    HASH_TABLE_BLOCK_TYPE *old_block_page =
         reinterpret_cast<HASH_TABLE_BLOCK_TYPE *>(buffer_pool_manager_->FetchPage(old_header->GetBlockPageId(block_idx)));
    for (size_t bucket_idx = 0; bucket_idx < BLOCK_ARRAY_SIZE; bucket_idx++) {
      if(old_block_page->IsReadable(bucket_idx))
      {
        Insert(nullptr, old_block_page->KeyAt(bucket_idx), old_block_page->ValueAt(bucket_idx));
      }
    }
    buffer_pool_manager_->UnpinPage(old_header->GetBlockPageId(block_idx), false);
    buffer_pool_manager_->DeletePage(old_header->GetBlockPageId(block_idx));
  }
